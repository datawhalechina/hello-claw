...
**Docker 部署**：适合生产环境，可实现环境隔离、版本管理、自动重启。详见[第十章](/cn/adopt/chapter10/)。
...
**第一层：一键上手**（第一章）——下载 AutoClaw，5 分钟拥有 AI 助手，零门槛体验 OpenClaw 的核心能力。

**第二层：手动安装与配置**（第二～四章）——学会手动安装、配置模型、接入聊天平台（QQ/飞书/Telegram）、掌握命令行基础操作，为进阶使用打好地基。

**第三层：深度使用**（第五～八章）——安装和管理技能、集成外部服务（Gmail/Notion/飞书）、生产环境部署、多模型与成本优化，把 OpenClaw 变成真正的效率工具。

**第四层：个人助理与专业应用**（第九～十一章）——定制个人助理系统、开发者效率工作流、故障排查与优化，充分发挥 OpenClaw 的潜力。

**第五层：构建 Claw**（第二部分）——从零实现一个简化版 AI Agent，深入理解 Agent Loop、工具调用、技能系统、记忆与身份系统的工作原理。
